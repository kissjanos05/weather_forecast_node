...
router.get('/weather', async function(req, res, next){
  try {
    var weather = await Weather.findOne({ where: {cityId: req.query.city_id, createdAt: {[Op.gt]: moment().subtract(10, 'minutes').toDate()}} })
    var city = await City.findByPk(req.query.city_id)
    if(!weather){
      var openWeatherData = await openWeatherApi.getCurrentWeather(city)
